...
class Qwen3VLReranker():
    ...
    def compute_scores(self, inputs):
        ...
        return scores.detach().squeeze(-1)
    ...
    def process(self, inputs, batch_size: int | None = None) -> list[float]:
        ...
        # Allow passing batch size either as an argument or inside `inputs`
        if batch_size is None:
            batch_size = inputs.get("batch_size", None)

        # Full-batch inference if batch_size is not set (or large enough)
        if batch_size is None or batch_size >= len(pairs):
            enc = self.tokenize(pairs)
            enc = enc.to(self.model.device)
            return self.compute_scores(enc)

        # Chunked batching to control VRAM usage
        final_scores: list[float] = []
        for start in range(0, len(pairs), batch_size):
            chunk = pairs[start:start + batch_size]
            enc = self.tokenize(chunk)
            enc = enc.to(self.model.device)
            final_scores.append(self.compute_scores(enc))

        final_scores = torch.cat(final_scores, dim=0)
        return final_scores
    
    def process_cross_query_batch(
        self,
        batch_inputs: List[dict[str, Any]],
        mini_batch_size: Optional[int] = None,
    ) -> List[List[float]]:
        """
        Compute reranking scores for multiple queries in a single batched run, without
        forming the full Cartesian product across queries.

        Parameters
        ----------
        batch_inputs:
            A list where each element is an `inputs` dict compatible with `process()`, e.g.
            {
        ...
        batch_size:
            If provided, process the flattened (query, document) pairs in chunks of this size.

        Returns
        -------
        scores_per_query:
            List of lists. scores_per_query[i][j] is the score for query i and its j-th document.
        """
        if not batch_inputs:
            return []

        # 1) Flatten only the required (q_i, d_{i,j}) pairs, preserving query-local order.
        flat_pairs = []
        doc_counts = []  # how many docs per query, used to unflatten results

        for item in batch_inputs:
            instruction = item.get("instruction", self.default_instruction)
            ...
            q_text = query.get("text", None)
            q_image = query.get("image", None)
            q_video = query.get("video", None)

            doc_counts.append(len(documents))

            for doc in documents:
                pair = self.format_mm_instruction(
                    q_text, q_image, q_video,
                    doc.get("text", None),
                    doc.get("image", None),
                    doc.get("video", None),
                    instruction=instruction,
        ...
        if not flat_pairs:
            # All queries were empty or had no documents
            return [[] for _ in batch_inputs]

        # 2) Batched inference over the flattened pairs.
        if mini_batch_size is None or mini_batch_size <= 0:
            mini_batch_size = len(flat_pairs)

        flat_scores: List[float] = []
        for start in range(0, len(flat_pairs), mini_batch_size):
            chunk = flat_pairs[start:start + mini_batch_size]
            enc = self.tokenize(chunk)
            enc = enc.to(self.model.device)
            flat_scores.extend(self.compute_scores(enc))

        # 3) Unflatten back to per-query lists.
        scores_per_query: List[List[float]] = []
        idx = 0
        for c in doc_counts:
            if c == 0:
                scores_per_query.append([])
            else:
                scores_per_query.append(flat_scores[idx:idx + c])
                idx += c

        return scores_per_query

def softmax_top_p_filter(
    ...
    *,
    ...
    """
    Softmax top-p filtering for reranking.

    Args:
        scores: 1D tensor/list of shape [N]. Prefer raw logits (pre-sigmoid).
        top_p: cumulative probability mass to cover (e.g., 0.9).
        temperature: softmax temperature (>0). Lower -> more peaked; higher -> flatter.
        min_keep: always keep at least this many documents.
        max_keep: cap the number of kept documents (e.g., <= k).
        return_details: if True, also return sorted scores/probs/cumsum.

    Returns:
        keep_idx: 1D tensor of indices into the original `scores`, sorted by descending score,
                  truncated to the chosen x satisfying the top-p rule (with min/max constraints).
        details (optional): dict with sorted_scores, sorted_probs, sorted_cumprobs, sorted_idx.
    """
    ...
    if scores.dim() != 1:
        raise ValueError(f"scores must be 1D (shape [N]), got shape {tuple(scores.shape)}")

    N = scores.numel()
    if N == 0:
        keep_idx = scores.new_empty((0,), dtype=torch.long)
        return (keep_idx, {}) if return_details else keep_idx

    # Sort by score descending
    sorted_scores, sorted_idx = torch.sort(scores, descending=True)

    # Stable softmax (subtract max); apply temperature on logits
    scaled = sorted_scores / temperature
    scaled = scaled - scaled.max()
    sorted_probs = torch.softmax(scaled, dim=0)
    sorted_cum = torch.cumsum(sorted_probs, dim=0)

    # Smallest x such that sum_{i<=x} probs >= top_p
    # We implement via mask and then enforce min_keep/max_keep.
    keep_mask = sorted_cum <= top_p
    keep_mask[0] = True  # always keep the top-1 even if it alone exceeds top_p
    x = int(keep_mask.sum().item())
    ...
    details = {
        ...
        "sorted_cumprobs": sorted_cum,
        "sorted_idx": sorted_idx,
    }
    return keep_idx, details